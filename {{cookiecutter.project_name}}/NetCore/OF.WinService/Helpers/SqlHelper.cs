...
namespace OF.WinService
{
    public abstract class SqlHelper
    {
        ...
        public static int GetMinID(string FieldName, string TableName, string ParentColumn = "", string parentId = "")
        {
            string strsql = "select min(" + FieldName + ") from " + TableName;
            List<SqlParameter> listParam = new List<SqlParameter>();
            if (!string.IsNullOrEmpty(ParentColumn))
            {
                strsql += string.Format(@" where {0}=@Parent ", ParentColumn);
                listParam.Add(new SqlParameter("@Parent", parentId));
            }
            SqlParameter[] param = listParam.ToArray();
            object obj = SqlHelper.GetSingle(strsql, param);
            if (obj == null)
            {
                return 0;
            }
        ...
        public static int GetMaxID(string FieldName, string TableName, string ParentColumn = "", string parentId = "")
        {
        ...
        public static void ExecuteSqlTran(Dictionary<string,SqlParameter[]> SQLStringList)
        {
            using (SqlConnection conn = new SqlConnection(ConnectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    ...
                    {
                        //循环
                        foreach (KeyValuePair<string,SqlParameter[]> kv in SQLStringList)
                        {
